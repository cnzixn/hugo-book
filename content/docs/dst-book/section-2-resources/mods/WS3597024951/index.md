---
title: "WS3597024951" 
weight: 2000
bookHidden: true
aliases:
  - "/p/WS3597024951"
workshop_fetched: true
chinese_draft_completed: true
processing_status: "completed"
---

## 景熹家居

（移植版）已知问题：

- 钢琴操作暂未支持...
- 部分音效“播放异常”，例如：钢琴曲、汽车电台，详情请看 [《模组音效》](/p/sound) 。

---

> 中文草稿：根据创意工坊描述整理，后续可继续校对。

为饥荒联机版加入一组家居、装饰和建筑内容，主要用于布置基地和改善居住氛围。工坊页面持续发布新家具与更新说明，具体物品、制作配方和兼容性建议以当前版本为准。

这是内容量较大的持续更新模组，下载和使用前建议查看工坊页面的最新更新日志，并在游戏中确认与其他建筑、美化类模组的兼容情况。

---

模组的网页版wiki：[--ModWiki--](https://wap.modwikis.com/mod?_id=695d178d53066f7643f484f6)

祝大家玩得开心！喜欢的小伙伴可以B站搜索并关注：画画的景熹 https://space.bilibili.com/5258940 。

---

[查看创意工坊原页面](https://steamcommunity.com/sharedfiles/filedetails/?id=3597024951)
